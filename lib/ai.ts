...
import { z } from 'zod';
import type { Commit, Prompt, ArchitecturalCallout, TokenUsage } from './types';
...
export const analysisResultSchema = z.object({
  ...
  keyDecisions: z.array(z.string()).describe('Key decisions made in this commit'),
...
function normalizeArchitecturalCallouts(
  ...
  return callouts.map((callout) => {
    ...
    let normalizedType: 'design-decision' | 'pattern-used' | 'performance-insight' | 'learning' =
      'learning';
...
export async function analyzeCommit(
  ...
  if (commit.fullDiff) {
    if (commit.fullDiff.length > maxDiffLength) {
      diffContent = commit.fullDiff.substring(0, maxDiffLength) + '\n... (diff truncated)';
    } else {

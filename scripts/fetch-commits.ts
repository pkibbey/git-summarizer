...
async function extractCommits(git: SimpleGit): Promise<Commit[]> {
  ...
  for (const logEntry of log.all) {
    ...
    const fileStats: { [key: string]: { additions: number; deletions: number } } = {};
    ...
    const enrichedFiles: Commit['files'] = files.map((f) => ({
      path: f.path,
      status: f.status === 'A' ? 'added' : f.status === 'M' ? 'modified' : f.status === 'D' ? 'deleted' : 'renamed',
...
async function fetchAndStoreCommits() {
  try {
    ...
    // Store to file
    await fs.writeFile(COMMITS_DATA_FILE, JSON.stringify(groupedObj, null, 2), 'utf-8');

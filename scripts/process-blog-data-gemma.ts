...
async function processBlogData(testMode = false) {
  try {
    ...
    console.log(`Starting blog data generation... ${modeLabel}\n`);
    ...
    const outputPath = path.join(process.cwd(), 'public', 'blog-data-gemma.json');
    ...
    if (testMode) {
      ...
      console.log(`\nNext steps:`);
      console.log(`   1. Review public/blog-data-gemma.json`);
      console.log(`   2. Commit changes: git add public/blog-data-gemma.json && git commit -m "chore: update blog data"`);
